...
def export_markdown_modular(endpoints: List[Dict[str, Any]], output_dir: str, project_name: str = "API") -> List[str]:
    """Exports clean 8-section modular Markdown documents: docs/<module>.md and docs/README.md."""
    generated = []
    grouped = group_endpoints_by_module(endpoints)
    docs_base = os.path.join(output_dir, "docs")
    os.makedirs(docs_base, exist_ok=True)

    # 1. Export per-module contract files (only 1 clean file per module: docs/<module>.md)
    ...
    # 2. Export Master Index: docs/README.md (clean links to ./{mod_name}.md)
    index_lines = []
    ...
    for mod_name, mod_endpoints in grouped.items():
        ...
        index_lines.append(f"| **`{mod_name}`** | {len(mod_endpoints)} ta | [📂 `{mod_name}.md`](./{mod_name}.md) | {sample_paths} |")

    index_lines.append("\n---\n")
    index_lines.append("## 📑 Umumiy Marshrutlar Ro'yxati (Global Endpoints Map)\n")

    for mod_name, mod_endpoints in grouped.items():
        index_lines.append(f"\n### 📦 Modul: [`{mod_name}`](./{mod_name}.md)\n")
        for i, ep in enumerate(mod_endpoints, 1):
            m = ep['method']
            p = ep['path']
            index_lines.append(f"{i}. [`{m}` **{p}**](./{mod_name}.md) — `{ep.get('handler', '')}`")
...
def export_typescript_modular(endpoints: List[Dict[str, Any]], output_dir: str) -> List[str]:
    """Exports clean modular TypeScript DTOs: types/<module>.d.ts and types/index.d.ts."""
    generated = []
    grouped = group_endpoints_by_module(endpoints)
    ...
    for mod_name, mod_endpoints in grouped.items():
        mod_flat_file = os.path.join(types_base, f"{mod_name}.d.ts")
